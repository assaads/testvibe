...
assumptions:
  ...
  - '[security] D1: _load_repro/base confines repro paths to the corpus dir (resolves symlinks too); add_entry enforces confinement at write time AND stores the repro RELATIVE to the corpus dir when colocated (round-trips confined; portable). Existing round-trip tests updated to expect the relative form — a behavior change required by the security property. quarantine_tests_for(base=...) and the plugin pass the corpus parent as base; an escaping repro is skipped, not executed. base=None preserves the legacy unconfined path for direct callers.'
  - '[simplification] D2: _cmd_run multi-kind set logic was dead code (run_scenarios adds at most ONE Failure); simplified to single-failure mapping (passed->0, kind==product->1, kind==infra->2). Behavior identical; existing run tests define the contract and stayed green.'
  - '[behavior-fix] D3: added _RC_USAGE=4 for caller/usage errors (wrong id, missing/corrupt corpus) in `corpus promote`; CorpusError now maps to rc 4 (was rc 2 _RC_INFRA). Genuine OSError during read/write still maps to rc 2 (infra). Pre-existing test_cli_corpus_promote_unknown_id_is_nonzero + a prior test_cli_corpus_promote_missing_corpus_is_infra updated to expect _RC_USAGE — the old conflated assertion encoded the bug D3 fixes. Documented the now-5-code taxonomy in the cli.py module docstring.'
  - '[concurrency] D4: _with_corpus_lock (fcntl.flock LOCK_EX on <path>.lock) wraps the read-modify-write in add_entry + promote_entry so concurrent writers serialize (no lost update). fcntl is Unix-only; guarded with try/except ImportError so non-Unix degrades to a no-op (project targets Linux — CI gate + dogfood /proc). Lockfile is left on disk after release (deleting under contention risks orphaning the fd). Two tests: a deterministic mechanism proof (second acquisition blocks until release) + a concurrency test proving both rows survive (was: last-writer-wins lost one row).'
